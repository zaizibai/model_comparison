...
parameters {
  vector[4] mu_pr; // group-level mean hyper parameter
  vector<lower=0>[4] sigma; // group-level standard deviation 
...
transformed parameters {
  ...
  vector<lower=-mu_pr[4]/sigma[4]> [ns] tau;           // strictly positive inverse temperature
  ...
  pi_  = mu_pr[3] + sigma[3] * pi_raw;
  tau = mu_pr[4] + sigma[4] * tau_raw;
  ...
  for (i in 1:ns) {
    ...
    for (t in 1:nt) {
      ...
      wv_g[cue[i, t]]  = qv_g[cue[i, t]] + pi_[i] * val[i,t];
